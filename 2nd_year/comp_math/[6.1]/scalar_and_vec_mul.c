...
void free_matrix(matrix& mat) {
    for (size_t i = 0; i < mat.col; ++i) free(mat.val[i]);
    free(mat.val);
}
...
void get_new_matrix(matrix& mat, char* pos, matrix* new_mat, int num) {
    char check = 1;
    int k = 0;
    if (!strcmp(pos, "row")) {
        ...
        for (size_t i = 0; i < new_mat->row; ++i, ++k) {
            ...
            (*new_mat).val[k] = (double*)malloc(sizeof(double) * new_mat->col);
            for (size_t j = 0; j < new_mat->col; ++j) {
                (*new_mat).val[k][j] = mat.val[i][j];
            }
        }
    }
    else {
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (check == '1') {
            ...
            if (mat.row > 2) {
                matrix new_mat = { 0, 0 };
                get_new_matrix(mat, pos1, &new_mat, num1);
